package remonone.nftilation.constants;

public class RequestConstant {
    
    public static final String REQ_PLAYER_LOG_IN = "https://nftilation-backend.onrender.com/api/v1/account/login/";
    public static final String REQ_FETCH_TEAMS = "https://nftilation-backend.onrender.com/api/v1/teams/";
    public static final String REQ_SEND_APPLY = "http://localhost:8000/api/v1/apply";
    public static final String REQ_GET_SKINS = "https://nftilation-backend.onrender.com/api/v1/skins";
    public static final String REQ_CONFIRM_DONATION = "http://localhost:8000/api/v1/confirm/";
    public static final String REQ_GET_TOKEN = "http://cryptobattles.fun/api/v1/token/";
}
