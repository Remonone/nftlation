...
public class ThirdTierPassiveIncome implements IPurchasableService {
    @Override
    public String getServiceName() {
        return "passive-income-third";
    }
    ...
    public void OnPurchase(Player buyer, int price) {
        ...
        Double requiredValue = (Double) NestedObjectFetcher.getNestedObject(MetaConstants.META_UPGRADES_PASSIVE, MetaConfig.getInstance().getUpgrades(), 1);
        ...
        Double nextLevelValue = (Double) NestedObjectFetcher.getNestedObject(MetaConstants.META_UPGRADES_PASSIVE, MetaConfig.getInstance().getUpgrades(), 2);
        ...
        team.getPlayers().forEach(playerModel -> {
            ...
            player.sendMessage(ChatColor.WHITE + playerName + ChatColor.GOLD + MessageConstant.TEAM_UPGRADE + MessageConstant.TEAM_UPGRADE_PASSIVE_INCOME + " " + 2);
        });
    }
}
